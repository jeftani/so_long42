...
/*   Updated: 2023/02/27 17:46:07 by ajeftani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **argv)
{
	...
	if (ac == 2)
	{
		...
		check_empty_lines(var.line);
		splited = ft_split(var.line, '\n');
		check_element(splited, &var);
		check_len(splited);
		check_walls(splited);
		var.map = ft_split(var.line, '\n');
		var.map2 = ft_split(var.line, '\n');
		var.map3 = ft_split(var.line, '\n');
		check_path(var.map, var.map2, var.map3);
